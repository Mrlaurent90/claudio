...
export default function Save( { attributes } ) {
	...
	const blockProps = useBlockProps.save( { className: 'ladb-trust-strip' } );
				...
				{ [ [ item1Icon, item1Title, item1Subtitle, 'item1' ], [ item2Icon, item2Title, item2Subtitle, 'item2' ] ].map( ( [ icon, title, subtitle, key ] ) => (
					<div key={ key } className="ladb-trust-strip__item">
						...
						<div className="ladb-trust-strip__text">
							<RichText.Content tagName="strong" className="ladb-trust-strip__title" value={ title } />
							<RichText.Content tagName="p" className="ladb-trust-strip__subtitle" value={ subtitle } />
						</div>
					</div>
				) ) }
			</div>
